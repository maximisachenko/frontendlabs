...
document.getElementById("scroll-event").addEventListener("scroll", function () {
    if (!scrollAlertTimeout) {
        ...
        setTimeout(function () {
            scrollAlertTimeout = false; // Через 10 секунд можно снова отображать уведомление
        }, 10000);
    }
});
...
document.getElementById("touch-event").addEventListener("touchstart", function () {
    this.style.backgroundColor = "#b3e5fc";  // Нежно-голубой цвет при касании
});

// 7. События, связанные с таймером: Реализован таймер с отсчетом времени, который можно ускорять, замедлять и останавливать.
document.getElementById("start-timer").addEventListener("click", function () {
    clearInterval(timer); // Остановка текущего таймера (если был)
    updateTimer(); // Запуск нового таймера
});
